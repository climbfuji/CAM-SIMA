# capgen_compat — CAM-SIMA compatibility layer for capgen
...
to capgen.  The end-state is an empty directory; every file here
retires as CAM-SIMA migrates its consumers to the agreed convergence
interface (next section).

CAM-SIMA owns this directory.  Capgen owns nothing in here.

## Interface convergence goal

The long-term goal is that CAM-SIMA interacts with capgen
...
1. **Command-line invocation** of three utilities:
   - `ccpp_validator.py` — returns success or failure with meaningful
     error messages.
   - `ccpp_capgen.py` — returns success or failure only.  Generated
...
convergence goal.  In particular: capgen's current
...
## Capgen surface this layer consumes (scaffolding inventory)
...
Re-export capgen symbols at the flat module paths CAM-SIMA imports
from today (`from ccpp_capgen import ...`, `from parse_tools import
...`, etc.).  Each retires the day CAM-SIMA updates the matching
imports to capgen's native paths — or, in the end-state, drops
...
| `parse_tools.py` | capgen's `metadata.parse_tools` symbols + `ParseObject` |
...
The actual capgen → original-capgen Python-surface translation.
...
capgen's internal Python state).
...
Capgen's parser is stricter and reads metadata into different
...
* `_rewrite_module_to_host` — rewrites `type = module` →
  `type = host` in-memory (capgen rejects the legacy spelling),
  ...
  filter requires).
* `_DROP_ATTRS` — strips per-variable attributes capgen doesn't
...
* Monkey-patches: `MetaVar.get_prop_value` plus six methods
  ...
  capgen `MetaVar` / `MetadataSection` instances under
...
* `find_scheme_names` — lightweight string scan (does not go through
  capgen's strict parser, so namelist-reader pseudo-schemes are
...
own scripts call into directly (NOT because capgen requires
them — capgen has no use for any of them).  These exist
because:
...
* `parse_object.py` inherits from a richer `ParseContext` than
  capgen exposes (writable `line_num`, region stack, etc.); the
  vendored `parse_source.py` carries that fuller `ParseContext` plus
  `ContextRegion` / `ParseContextError` / `ParseSource`.

| File | What it carries |
|---|---|
| `parse_source.py` (409 LOC) | Original-capgen-style `ParseContext` (writable, with region stack); `ContextRegion`; `ParseContextError`; `ParseSource`; `context_string` and a few utility helpers.  `CCPPError` / `ParseSyntaxError` / `ParseInternalError` are re-exported from capgen (not vendored) so the exception identity is shared. |
...
`capgen_compat/` itself relies on capgen being importable, so the
pinned `ccpp_framework/capgen/` external is also appended to
`sys.path`.  `cam_autogen.py` adds both.

Note one consequence of the `capgen-ng` → `capgen` rename: the
generator's entry-point module is now `ccpp_capgen.py`, the *same*
name as this layer's shim (`capgen_compat/ccpp_capgen.py`).  Because
the compat directory is at the front of `sys.path`, a bare
`import ccpp_capgen` always resolves to the shim — which is what
CAM-SIMA wants, but means the shim cannot reach the real generator by
name.  `_runner.py` therefore loads `ccpp_capgen` explicitly from the
`ccpp_framework/capgen/` `sys.path` entry (see `_real_capgen()`).
Before the rename the generator was `ccpp_capgen_ng` and no clash
existed.

## Phased retirement plan

End-state: this directory does not exist.  CAM-SIMA's autogen
pipeline interacts with capgen only through the three CLI
...
| **B** | Rewrite `write_init_files.py` to consume `datatable.xml` only (no `cap_database` Python object).  Drop `FortranWriter` in the rewrite — use direct f-strings or a small in-tree emitter. | `_cap_database.py`, `_var_wrapper.py`, `_runner.py`, the `MetaVar` / `MetadataSection` monkey-patches, **`fortran_write.py`** if it was the last `FortranWriter` caller, and the `return_state=True` hook on `ccpp_capgen.capgen` (delete on the capgen side) |
| **C** | Rewrite `generate_registry_data.py` to use capgen's CLI + a small Fortran emitter | rest of `metadata_table.py`'s parse-shim, **and `fortran_write.py`** if it had not already gone |
...
| **E** | Mechanical sweep: replace `from ccpp_capgen import …` / `from framework_env import …` / etc. with capgen's native paths, or remove them entirely if the CLI replaces them | Remaining flat-module shims |
...
1. **Delete the `return_state=True` kwarg** on
   `ccpp_capgen.capgen` once Phase B lands.  No production caller
   should ever depend on it.
2. **If CAM-SIMA cannot avoid Python imports**, capgen exposes
